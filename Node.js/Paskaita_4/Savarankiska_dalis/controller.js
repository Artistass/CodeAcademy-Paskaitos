...
export function sameCarModel (req, res) {
  const params = req.params;
  const car = params.car;
  const user = data.filter((u) => u.car === car)
...
export function getAllFemales(req, res) {
  const namesOfFemales = [];
  data.forEach((user) => {
    if (user.gender === 'Female') {
      namesOfFemales.push(user.first_name + ' ' + user.last_name);
    }
  });
  res.json(namesOfFemales);
}
